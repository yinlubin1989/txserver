...
import { ACTIONS, BattleGame, clearPresses, createInputState, normalizeInput } from "./public/iron-circuit-clash/engine.mjs";

const dev = process.env.NODE_ENV !== "production";
const hostname = process.env.HOSTNAME || "127.0.0.1";
const port = Number.parseInt(process.env.PORT || "3000", 10);
const websocketPath = "/ws/iron-circuit-clash";
...
function createRoom(client) {
  ...
  const room = {
    ...
    inputs: { 1: createInputState(), 2: createInputState() },
    game: null,
    loop: null,
    lastTick: 0,
    sequence: 0,
...
function startMatch(room) {
  ...
  room.inputs = { 1: createInputState(), 2: createInputState() };
  ...
  room.loop = setInterval(() => {
    ...
    clearPresses(room.inputs[1]);
    clearPresses(room.inputs[2]);
